                        ...
                        if (userData.books && userData.books.length > 0) {
                            userData.books.forEach(async bookData => {
                                ...
                                bookElement.innerHTML = `
                                    ...
                                    <p>Rating: <span class="ratingDisplay">${bookData.rating !== null ? bookData.rating : 'Not rated'}</span></p>
                                ...
                                const submitRatingButton = bookElement.querySelector('.submitRatingButton');
                                    ...
                                    if (!isNaN(newRating) && newRating >= 1 && newRating <= 5) {
                                        try {
                                            const updatedBooks = userData.books.map(book => {
                                                if (book.title === bookData.title && book.author === bookData.author) {
                                                    return { ...book, rating: newRating };
                                                }
                                                return book;
                                            });
                                            await updateDoc(userDocRef, { books: updatedBooks });
                                            const ratingDisplay = bookElement.querySelector('.ratingDisplay');
